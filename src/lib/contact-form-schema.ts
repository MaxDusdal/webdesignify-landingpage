...
export type MarketingTool = typeof FORM_OPTIONS.marketingTools[number]['value'];
...
export type ReferralSource = typeof FORM_OPTIONS.referralSources[number]['value'];
...
export const contactFormSchema = z.object({
  ...
  industry: z.enum([...FORM_OPTIONS.industries.map(i => i.value)] as [string, ...string[]]),
  businessSize: z.enum([...FORM_OPTIONS.businessSizes.map(i => i.value)] as [string, ...string[]]).default('1-10'),

  // Step 3: Current Infrastructure
  hasWebsite: z.enum([...FORM_OPTIONS.hasWebsite.map(i => i.value)] as [string, ...string[]]),
  websiteUrl: z.string().optional(),
  currentMarketing: z.array(
    z.enum([...FORM_OPTIONS.marketingTools.map(i => i.value)] as [string, ...string[]])
  ).optional().default([]),

  // Step 4: Project Details
  projectType: z.array(
    z.enum([...FORM_OPTIONS.projectTypes.map(i => i.value)] as [string, ...string[]])
  ).min(1, { message: 'Bitte mindestens eine Option auswählen' }),
  timeline: z.enum([...FORM_OPTIONS.timelines.map(i => i.value)] as [string, ...string[]]),
  budget: z.enum([...FORM_OPTIONS.budgets.map(i => i.value)] as [string, ...string[]]),

  // Step 5: Additional Information
  message: z.string().optional(),
  howDidYouHear: z.enum([...FORM_OPTIONS.referralSources.map(i => i.value)] as [string, ...string[]]),
}); 